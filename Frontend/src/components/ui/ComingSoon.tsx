...
const ComingSoon: React.FC = () => {
    ...
    return (
        <div className={`flex flex-col items-center justify-center min-h-[60vh] text-center px-4 transition-all duration-300 ${theme === 'dark' ? 'text-slate-100' : 'text-slate-900'
            ...
            <div className={`p-6 rounded-full mb-8 relative ${theme === 'dark' ? 'bg-blue-500/10' : 'bg-blue-100'
                }`}>
                <div className="absolute inset-0 rounded-full animate-ping opacity-25 bg-blue-500"></div>
                <Rocket className={`w-16 h-16 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'
                    }`} />
            </div>

            <h1 className="text-4xl md:text-5xl font-extrabold mb-4 tracking-tight">
                Coming <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-indigo-500">Soon</span>
            </h1>
            ...
            <div className="mt-16 grid grid-cols-3 gap-8 w-full max-w-lg opacity-40">
                ...
                <div className={`h-1 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(37,99,235,0.8)]`}></div>
                <div className={`h-1 rounded-full ${theme === 'dark' ? 'bg-slate-800' : 'bg-slate-200'}`}></div>
            </div>
        </div>
    );
